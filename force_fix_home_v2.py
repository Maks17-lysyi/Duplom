          ...
          <!-- Visual Slots Footer -->
           <div class="slot-container">
             {% for slot_user in lobby.get_slots %}
               {% if slot_user %}
                 <!-- Filled Slot: Rectangular Card -->
                 <div class="slot-card filled" title="{{ slot_user.username }}">
                   <div class="slot-avatar">
                      {% if slot_user.gamer_profile.avatar %}
                        <!-- Place real avatar logic here later -->
                        {{ slot_user.username|slice:":1"|upper }}
                      {% else %}
                        {{ slot_user.username|slice:":1"|upper }}
                      {% endif %}
                   </div>
                   <div class="slot-name">{{ slot_user.username }}</div>
                 </div>
               {% else %}
                 <!-- Empty Slot -->
                 <div class="slot-card empty">
                    ...
                    {% else %}
                       <div class="slot-join-btn" style="cursor: default;">
